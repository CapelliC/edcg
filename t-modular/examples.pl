...
acc_info(adder, I, In, Out, plus(I,In,Out)). % adder
...
pred_info(flist_ssu,1,[fwd]).
pred_info(rlist,1,[rev]).
pred_info(rlist_ssu,1,[rev]).
pred_info(sum_first_n,1,[adder]).
pred_info(sum_first_n_ssu,1,[adder]).
pred_info(sum,0,[adder,dcg]).
pred_info(expr_code,1,[size,code]).
pred_info(expr_code_ssu,1,[size,code]).
